# 阅读状态

正常阅读只有“读取当前原文并逐层分析”这条主线。用户追问或回看不创建新 Session；不维护任何训练模式或训练结果对象。

## 一份状态足够

每篇论文保留一个 Primary Issue。正文指向唯一的当前 `[READING STATE]`；状态至少包含：

```text
论文/Revision + reading-mcp document/hash/segmentation identity
当前已授权范围 + 已揭示位置/精确 TextLocator
待完成单元的 locator + 下一阶段（仅有未完成动作时记录）
分析契约 id + contract_source_commit（完整 commit SHA，旧 Profile 如存在则保留）
简短前文模型：已知事实、关键连接（依据与条件）、尚未回答的问题
当前状态/阻塞 + 下一动作
```

这是字段语义，不是新增 JSON Schema。复用已有可靠绑定与完整 locator，不要求用户填写。事实和推论在摘要内能区分即可；无内容的字段不必机械列出。

操作定位和分析摘要可以放在同一记录的两个区块，语义分开，不强制拆成两份文件。只有摘要明显过长或用户要求反复查阅时，才抽出一份资产并链接；不为每句或每个模式增加独立对象。

## 已揭示与已完成

`revealed_position` 记录已实际暴露的位置，不等于已经完成分析。枚举返回单元 N 后，在同一状态记下其完整 locator 和待完成阶段（精确回读或分析）；输出分析并更新模型后才清除待完成动作。失败或交接时随状态保存，不新增逐句文件。

例如：N 已枚举但回读失败 → revealed position 保留 N、下一动作是回读 N → 恢复后精确读取并分析 N → 本次输出结束。不得把 N 当 forward anchor 直接取 N+1。仅摘要恢复也不能冒充已经向用户完成了这次分析；输出是否完成不明时重复 N 的分析比跳过更可靠。

旧记录无待完成字段不自动证明已完成；核对其失败记录、摘要覆盖与下一动作。无法确认时保留缺口并恢复对应已知单元，不能倒退已揭示位置或猜测跳过。

## 何时保存

同一会话逐句更新当前位置；自然段落/机制收束、暂停、交接、范围变化或故障时再写 GitHub。没有这些变化不制造新 comment。用户要求保存时立即保存。

写入完整新状态后，更新 Issue 正文指针并回读确认；旧状态留作历史。保存完成须同时核对 comment URL、Source identity、locator、范围、契约 commit、摘要覆盖位置及下一动作；正文若保留这些字段的投影，也必须一致。写 comment 成功而 body 未更新只能报告部分保存，不能称交接完成。重试先定位已写入的状态，避免追加重复记录。若写入中断导致指针落后，按 [恢复规则](../workflows/issue-driven-workflow.md#恢复与并发) 检查较新控制记录，不能按旧指针盲目前进。

不保存完整 transcript，也不声称未持久化的会话状态可以无损恢复。

## 范围与版本

`planned_scope` 是既有历史计划；`current_scope_boundary` 是当前实际授权，不因简化删除。下一单元越界前先获用户许可并保存变化；若用户明确授权整节，则不用逐句再申请授权。

Source/normalization 变化不自动迁移旧 locator；分析契约切换记录一次旧/新绑定、生效位置与依据。已读位置只增不减。回看只改变视图，不把看过的内容变回未知。

契约身份为 `id + contract_source_commit + path`；`main`、PR 编号或单独的 `v2` 不足以固定规则。Issue 只保存该绑定和协议链接，不复制风格清单。同 id 下改变呈现也须记录新 commit 和生效 locator。历史记录没有 commit 时标为未记录，不根据时间猜测；在明确授权采用新规则后记录一次显式绑定，保留历史缺口。

历史 Session 的 completed/abandoned/contamination 保留；本轮阅读结束不意味着论文永久学完，不追加作答或验收阶段。

## 规则版本加载

在任何正文调用前，先加载当前仓库的操作规则（AGENTS、Skill、Source Adapter、状态与 Issue 工作流），再按状态的 `contract_source_commit + path` 读取分析协议。首次启动使用已核验 main 的完整 SHA 建立分析绑定；已有会话继续用原绑定。只加载本动作需要的规则，同会话复用已核验版本。

新会话恢复已有绑定也须实际加载该固定协议，以及它指定的完整呈现示范（同一 commit）；历史状态中的“已加载”只描述原执行会话，不代表当前会话已经取得这些内容。只有当前会话实际加载并保有的材料才可复用。旧契约未指定示范时不擅自添加新版本要求；文件不可用按上述绑定缺失规则停止，不退回 main。用户只说“继续阅读论文 #N”就应触发完整恢复，无须重复风格提示。

当前操作规则负责范围、身份、恢复和工具调用；固定分析协议负责解释内容、背景与呈现。旧分析协议里的工具步骤不覆盖当前操作限制；若两者不能同时满足，停止正文调用并说明具体冲突，按已有用户授权显式切换，否则请求选择。固定文件缺失或 commit 不明时不退回 main 猜读。

普通阅读默认允许的背景见绑定协议的 Source 边界；用户有更窄限制或明确授权的额外背景时，在同一状态保留该差异，首次保存可注明采用协议默认。不为通用概念逐项申请授权，也不扩大旧会话已记录的限制。

## 分析摘要

摘要只帮助 AI 保持已读模型、分析衔接和开放问题；放在同一状态里即可。旧记录中的 Learning Artifact 只是历史命名，不是必须创建的对象、答案库或新的验收任务。历史固定候选按其原 commit 保留，不为继续阅读重新处理。

摘要压缩表述，不压掉成立条件：保留“哪条已读信息，在什么前提下，支持哪个判断”，并关联必要的已读 Source 引用。不要只留下术语或最终结论，使续读时必须重新猜测理由；未解决的连接继续标为未知，不为了补全摘要读取后文。这仍是一份状态，不新增字段、文件或验收步骤。

摘要不是最近一段的滑动窗口。更新时保留仍支撑理解的较早机制、它们的连接与未决问题；新增局部模型不应整段替换累计模型。开放问题由后续已读单元回答时，记录“原来未知什么 → 哪个单元补充了什么”，只关闭实际回答的部分。无需保存完整分析或全部历史细节。

恢复时分别核对定位连续性与模型充分性：能找到下一单元，不等于已经恢复理解它所需的前文。当前单元取得后，在输出前从已恢复模型中寻找它回答或修正的相关机制和开放问题，不只回接紧邻上一句；若发现关键依据缺失，先按下段在已读范围内补齐，再完成同一单元，不额外枚举后文。输出仍由固定分析协议控制，不把这些内部核对项变成用户要填写的表。

摘要覆盖落后于已揭示位置时，分别标明可靠摘要截止点与缺失范围，保留较新的真实 locator。先从同身份的既有记录恢复；仍缺失时，仅在已读边界内经 reading-mcp 精确回读必要内容以重建摘要，再推进新正文。工具不可用则保留恢复缺口；不倒退 revealed position、不把旧摘要冒充完整模型，也不以 Issue 分析代替 canonical 原文。正常完整状态不增加此恢复步骤。
